...
class TaskManager:
    ...
    def __init__(self, args, tasks: List[Dict[str, Any]]):
        self.single_prompt = args.end_to_end_prompt
        self.tasks = tasks
        self.use_sequential_tasks = args.planner_guided
        self.task_timeout = args.task_timeout
        self.current_task_idx = 0
        self.current_prompt = tasks[0]["prompt"] if tasks and args.planner_guided else self.single_prompt
    ...
    def check_task_timeout(self, step: int) -> bool:
        ...
        if self.use_sequential_tasks:
            ...
            if steps_on_current_task >= self.task_timeout:
                ...
                logging.info("🛑 Episode terminated early due to task timeout")
                return True
        else:
