...
def run_robosuite_with_openpi(args: Args) -> None:
    ...
    for ep in range(args.episodes):
        ...
        logging.info(f"Task manager state: current_task_idx={task_manager.current_task_idx}, use_sequential_tasks={task_manager.use_sequential_tasks}")
        ...
        while t < args.horizon:
            ...
            if t % 50 == 0:
                if not task_manager.use_sequential_tasks:
                    ...
                    logging.info(f"Step: {t}/{args.horizon}, Reward: {reward:.3f}, Completed: {completed_tasks}/{total_tasks}, Working on task {current_task_num}, Steps on current task: {steps_on_current_task}, Loop Time: {time.time() - loop_start_time:.4f}s")
                else:
