...
class DataConfig:
    ...
    # If true, will prevent downloading from Hugging Face and use only locally cached datasets
...
class TrainConfig:
    ...
    # If set, overrides any dataset/task prompt so the model sees a single instruction.
    # Example: "Play Towers of Hanoi"
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("/home/hrilab/Documents/.vlas/cycliclxm-slim/CyclicLxM/openpi/checkpoints/pi0_hanoi_base/base_finetune/29999/params"),
        num_train_steps=30_000,
        # The freeze filter defines which parameters should be frozen during training.
        # We have a convenience function in the model config that returns the default freeze filter
        # for the given model config for LoRA finetuning. Just make sure it matches the model config
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            ...
            assets=AssetsConfig(
                asset_id="tduggan93/height_stacking",  # This should match the assets directory structure
            ),
            base_config=DataConfig(
                prompt_from_task=True,
    ...
    TrainConfig(
        name="pi0_hanoi_planner_guided_300",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
